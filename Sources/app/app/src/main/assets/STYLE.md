...
These two files (`server_webui.html`, `server_docs.html`) are the bundled UI for the embedded HTTP server. The `:server` process serves them as static strings at `/`, `/webui`, and `/docs`. **They run in third-party browsers on the LAN — they must be self-contained, fast, and indistinguishable in feel from the rest of FRIDAY AI.**
...
Both palettes are aligned with the marketing site (`/home/home/WebstormProjects/FRIDAY AI-Web`). If you change the website palette, update these too — the user notices.
