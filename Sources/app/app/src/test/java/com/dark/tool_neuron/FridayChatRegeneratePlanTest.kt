...
// Pure companion-fn tests for FridayChatViewModel.regenerate() — proves the
// history we send to brainContinue actually excludes the answer being replaced.
class FridayChatRegeneratePlanTest {
    ...
    fun regenerate_excludesAssistantBeingReplaced() {
        ...
        val (history, existing) = com.dark.tool_neuron.viewmodel.FridayChatViewModel.planRegeneration(turns)
    ...
    fun regenerateEmptyConversationStaysEmpty() {
        val (history, existing) = com.dark.tool_neuron.viewmodel.FridayChatViewModel.planRegeneration(emptyList())
