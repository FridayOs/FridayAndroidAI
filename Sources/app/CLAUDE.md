...
`VoiceRouter.route()` resolves the active Voice Gateway per turn via a pure `decide(voice, brain)` companion fn (unit-tested). `VoiceRoute.LOCAL` → `LocalBridge(voice, brain)`: on-device STT produces a transcript, `brainTurn` runs the reply, on-device TTS speaks it. `VoiceRoute.CLOUD` (Gemini Live / OpenAI Realtime) → `CloudUnavailable` because the bidirectional realtime-audio socket is **not** part of M2-02 — the screen surfaces an honest "coming soon" instead of silently falling through to local STT under a Gemini/OpenAI label. No voice gateway → `NoVoice`; CLOUD picked with no brain → `NoBrainForCloud`. `VoiceBridge.kt` holds the voice↔brain contract types (`VoiceTurn`, `CloudVoiceCapability`). `FridayVoiceViewModel` calls `voiceRouter.route()` in `startListening()` and only asks for the mic on `LocalBridge`; every other route surfaces its reason before recording.

### VM wiring

- `FridayChatViewModel` routes `send` through `router.brainTurn(history)`; a missing brain surfaces `router.brainUnavailableMessage()`.
- `FridayVoiceViewModel` first asks `voiceRouter.route()` in `startListening()` — only `LocalBridge` opens the mic; `CloudUnavailable` / `NoVoice` / `NoBrainForCloud` surface clear copy before any recording. `stopListening()` always bridges audio→`router.brainTurn`; audio handling and reasoning are split and the brain is required even when voice is local. `cancel()` cancels the flow Job, calls `router.brainCancel()`, then stops speaking/recording.
...
Unit (JVM, off-device): `GatewayValidationTest`, `GatewayErrorSanitizerTest` (proves no secret leaks), `GatewayProviderRoleTest`, `GatewayRoleRouterTest` (brain cloud/local/unavailable decision + no-FRIDAY-API guarantee), `VoiceRouterTest` (local-bridge / cloud-unavailable / no-voice / no-brain-for-cloud + cloud-voice-never-implies-FRIDAY-API). Instrumented: `GatewayConfigRepositoryTest` (CRUD, restart persistence, multi-instance same provider, status persistence, sanitized-error-not-key, independent brain/voice, brain-only-never-voice, active-delete fallback, no-provider state) — **run green on a real SM-A325F (API 33), `OK (8 tests)`**.
...
- Don't fall through to local STT/TTS when the user picked a CLOUD Voice Gateway. Cloud realtime-audio is `CloudUnavailable` until the bidirectional socket ships; the screen surfaces "coming soon" rather than pretending the local path is the cloud pick.
