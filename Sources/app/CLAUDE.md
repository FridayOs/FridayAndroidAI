...
- Don't strip the four `brain_*` ops off `LiveCloudBridge` or route them through anything but the injected `BrainBridge`. Gemini Live is audio-only transport; reasoning/tools/actions go to the Brain Gateway via `brainTurn`/`brainContinue`/`brainCancel`/`brainConfirm`, never a FRIDAY/Firebase proxy and never the audio path. Each turn carries the session's stable `sessionId` + a fresh `correlationId` so `brainCancel`/`brainConfirm` act on the turn the latest `brainTurn`/`brainContinue` opened (barge-in cancels the right turn). `LiveCloudBridgeTest` proves all four ops + correlation against a fake brain.
...
- Don't route the Gemini Live transcript to a model any way but the Brain Gateway bridge. `LiveCloudBridge` (the FRI-530 CloudBridge seam) exposes the four `brain_*` ops — `brainTurn(history,transcript)`, `brainContinue(history)`, `brainCancel()`, `brainConfirm()` — over the `BrainBridge` interface, so the live transport produces a transcript and the brain does all reasoning/tools/actions. The bridge never opens a model socket itself and the brain never sees audio. Every voice interaction carries a stable `sessionId`; each `brainTurn`/`brainContinue` mints a fresh `correlationId` tagged `sessionId:<uuid>` so `brainCancel`/`brainConfirm` resolve against the right turn. `LiveVoiceSession.cancel()` calls `brainCancel()` before `engine.cancel()` so audio and the in-flight brain turn stop together.
