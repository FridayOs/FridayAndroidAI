...
- `GeminiLiveConfig.kt` — resolved wire config from a `GatewayConfig` (apiKey/model/voice/locale/bargeIn + host/port/path). `wireModel` prefixes `models/`; `from(GatewayConfig,…)` parses `baseUrl` into `baseHost`/`basePort`/`basePath` via `URI` (a blank/invalid/non-TLS endpoint falls back to the secure default), defaults model `gemini-2.5-flash-native-audio-preview-12-2025` / voice `Aoede` / port 443.
...
- `BrainGatewayLiveAdapter.kt` — `@Singleton` production `LiveBrainGateway` adapting the correlation-carrying seam onto the app's real `BrainBridge` (`GatewayRoleRouter`). Tracks the full active `BrainCorrelation` (session + turn) in an `AtomicReference`; `brainCancel`/`brainConfirm` CAS the active correlation to null so only an exact-match cancel/confirm forwards — a superseded turn OR a foreign session is dropped (cross-session isolation). The underlying brain's cancel is turn-scoped (collecting Job / armed gate); the adapter OWNS correlation since `BrainBridge` is identity-less by contract.
- `LiveCloudBridge.kt` — the **FRI-530 CloudBridge**: routes the live transcript to the `LiveBrainGateway` seam via the four `brain_*` ops. `brainTurn(history,transcript)` appends the transcript as the new user turn + mints a fresh `BrainCorrelation` under the stable `sessionId`; `brainContinue(history)` re-runs with a fresh turnId; `brainCancel()` atomically takes (`getAndSet(null)`) the in-flight correlation; typed `cancelTurn`/`confirmTurn` use CAS on an `AtomicReference` so a stale correlation (superseded turn / foreign session) can never clear or resolve a newer turn — even racing against `open()`. **Audio never crosses this seam — only text.**
...
- Don't revert `GeminiLiveConfig.DEFAULT_MODEL` to `gemini-2.0-flash-live-001` — that model was shut down 2025-12-09 (Google deprecations). The default is `gemini-2.5-flash-native-audio-preview-12-2025`, the current live-audio model on the `generativelanguage.googleapis.com` BidiGenerateContent endpoint (verified against ai.google.dev's live-model list). `gemini-3.1-flash-live-preview` is NOT a real published id — don't use it. Bump only to a model that appears on the official live-model list when Google rotates again; a dead default means the out-of-box voice path can't connect.
