...
- `LiveAudioPlayer.kt` — `@Singleton` `LiveAudioSink`, 24kHz PCM16 `AudioTrack` MODE_STREAM. Writes run on a dedicated single-consumer coroutine (a barge-in `flush()` never waits behind an in-flight `WRITE_BLOCKING` on the receive thread). `flush()` bumps a generation token + pause/flush/play so a barge-in/interrupt drops the current turn's queued audio instantly; `enqueue(pcm,gen)` drops chunks from a superseded generation.

Orchestration:
- `LiveSessionEngine.kt` — `internal` class owning one session as a cold `run(config): Flow<LiveEvent>`. Connect→send setup on onReady→CONFIGURED on `setupComplete`→start mic→STREAMING on first audio, with `LiveRetryPolicy` bounded reconnect on transient failures (backoff budget resets once an attempt reaches CONFIGURED). Non-local returns from `collect` are illegal, so a server-reported failure is captured via `AtomicReference` and closes the socket; `run()` closes the `callbackFlow` producer when the retry loop ends so a terminal session completes for collectors instead of hanging on `awaitClose`. Clean WS close (1000/1005) ends without retry; mic denial returns terminal `PERMISSION` (never idles into a TIMEOUT loop). **Lifecycle contract** (FRI-548 owns transport/session teardown; FRI-562's UI calls these): `onAudioFocusLost`/`onBackground` → stop capture + flush playback + full teardown (mic never held off-screen), `onAudioRouteChanged` → flush queued playback only and keep streaming (headset/BT/speaker swap), `onPermissionRevoked` → terminal ERROR + teardown, `endUserTurn` → stop capture + `audioStreamEnd`, `cancel`/`teardown` → stop capture+playback+socket+scope (terminal ERROR never masked by CLOSED). `classifyTransport` maps socket/handshake exceptions → `LiveErrorKind`.
...
- `BrainGatewayLiveAdapter.kt` — `@Singleton` production `LiveBrainGateway` adapting the correlation-carrying seam onto the app's real `BrainBridge` (`GatewayRoleRouter`). Tracks the active `turnId` and drops a cancel/confirm whose `turnId` isn't the active one; the underlying brain's cancel is already turn-scoped (collecting Job / armed gate).
- `LiveCloudBridge.kt` — the **FRI-530 CloudBridge**: routes the live transcript to the `LiveBrainGateway` seam via the four `brain_*` ops. `brainTurn(history,transcript)` appends the transcript as the new user turn + mints a fresh `BrainCorrelation` under the stable `sessionId`; `brainContinue(history)` re-runs with a fresh turnId; `brainCancel()`/`brainConfirm()` act on the **current** turn. Typed `cancelTurn(correlation)`/`confirmTurn(correlation)` **stale-guard**: a cancel/confirm for an already-superseded turn (or a foreign session) is dropped, so a late barge-in never cancels the wrong turn. **Audio never crosses this seam — only text.**
- `LiveVoiceSessionFactory.kt` — `@Singleton`, injects `LiveAudioCapture`/`LiveAudioPlayer` (as the `LiveAudioSource`/`LiveAudioSink` seams) + the `LiveBrainGateway`. `create(GatewayConfig,voice,locale,bargeIn)` mints one `sessionId` and returns a `LiveVoiceSession` pairing the engine (over the `LiveTransport` seam) with a `LiveCloudBridge(brain, sessionId)`; exposes `run()`/`endUserTurn()`/the lifecycle passthroughs/`cancel()` (cancels the in-flight brain turn then the engine). `supports()` gates to `GatewayWireFormat.GEMINI`. This is the injectable seam FRI-562's Voice Home calls; it owns transport/session + brain-bridge wiring, never the brain reasoning or the Android action layer.

### Tests (JVM, off-device)

`LiveProtocolTest` (17 — setup/audio/end framing, server-frame parse + interrupt-before-turnComplete ordering, malformed→Unknown, error taxonomy), `WebSocketFrameTest` (14 — mask bit, roundtrip, 7/16/64-bit lengths, fragmentation, close parse, **RFC 6455 validation: masked server frame / RSV set / reserved opcode / fragmented control / >125-byte control all → `ProtocolException`**), `WebSocketHandshakeTest` (7 — Accept digest, 101 validation, status extraction), `LiveRetryPolicyTest` (8 — only transient loops, bounded backoff, quota never loops), `GeminiLiveConfigTest` (10 — host/port/path parse from the user URL, model prefixing, **official default model**, endpoint path carries base path + key, **non-TLS/invalid endpoint falls back to the secure Gemini default**), `LiveSessionEngineClassifyTest` (10 — auth/model/quota/network mapping), `LiveJsonTest` (4 — escaping/no-secret-leak + parse), `LiveCloudBridgeTest` (10 — all four brain_* ops deliver the BrainCorrelation to a fake LiveBrainGateway seam, transcript appended as the user turn, stable sessionId + fresh turnId per turn, cancel/confirm forward the active turn's correlation, stale cancel/confirm for a superseded turn is dropped, foreign-session cancel ignored), `LiveSessionEngineLifecycleTest` (11 — audio focus loss/background full teardown, route change flush-only, permission revoke terminal ERROR, idle-engine rapid lifecycle safe, **real running-session rapid start/stop no-leak**, **mic-fails-after-open → terminal AUDIO not retry loop**, network drop→recovery reconnects to CONFIGURED, auth non-retryable, clean collector-cancel tears down transport+audio, mic-denied-mid-session → PERMISSION not TIMEOUT loop), `LiveWebSocketTransportStreamTest` (5 — **the REAL `LiveWebSocketTransport` over a loopback socket**: producer completes + frames arrive in order on a normal peer close, completes on a bare EOF — the awaitClose-hang regression, **a UTF-8 code point split across two fragments reassembles as one glyph**, **a post-handshake send failure tears the transport down instead of being swallowed**, **`hardenTls` enables HTTPS endpoint identification**), `BrainGatewayLiveAdapterTest` (6 — **the production `BrainGatewayLiveAdapter` over a fake of the app's real `BrainBridge`**: turn/continue forward, cancel/confirm forward for the active turn only, superseded-turn drop, and **cross-session isolation** — a same-turnId-different-session correlation never touches the active turn). All green, 0 skipped, in `./gradlew :app:testDebugUnitTest`.
...
- Don't revert `GeminiLiveConfig.DEFAULT_MODEL` to `gemini-2.0-flash-live-001` — that model was shut down 2025-12-09 (Google deprecations). The default is `gemini-3.1-flash-live-preview`. Bump it from the official live-model list when Google rotates again; a dead default means the out-of-box voice path can't connect.
