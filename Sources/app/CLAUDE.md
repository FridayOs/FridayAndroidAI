...
Rebrand target (M1-01, 2026-07-07): app name `FRIDAY AI`, assistant name `Friday`, package id `com.friday.ai`. Java/AIDL source tree stays under `com/dark/tool_neuron` for now — renaming the Kotlin package would force a coordinated JNI / native / asset migration and was scoped out. The Gradle namespace and `applicationId` are the only runtime identity; the `package` declarations and directory layout remain `com.dark.tool_neuron` until a dedicated M1 follow-up re-homes the sources. New code targets `com.friday.ai.R` for the R class; everything else still resolves via the un-rewritten source tree.

Repository layout (final, M1-01):

- `/home/leo/data/Projects/friday-android/Sources/app/` — Android Gradle root (`settings.gradle.kts`, `gradlew`, `build.gradle.kts`, `gradle/`, `app/`, `hxs/`, `hxs_encryptor/`, `networking/`, `download_manager/`, `native-server/`, `plugin-api/`, `plugin-exc/`, `plugins/`, `libs/`, `CLAUDE.md`).
- `/home/leo/data/Projects/friday-android/Sources/app/app/` — main Android application module (`:app`).
- `/home/leo/data/Projects/friday-android/Sources/api/` — backend API sibling. Not part of the Android Gradle build; lives outside `Sources/app/`. Android persists locally with HXS; the backend exists to serve auth, profile metadata, push routing, policy, and health metadata only. User-owned AI gateways are called **directly from Android** — there is no FRIDAY API proxy mode in this phase; the FRIDAY-hosted provider is a disabled placeholder for a later phase.

All Gradle commands in commands, READMEs, plan comments, and task notes must use `Sources/app/` as the root. `cd Sources/app && ./gradlew ...` is the canonical entry point.
...
Package: `com.friday.ai` (Gradle `namespace` + `applicationId`); Kotlin/AIDL source tree still rooted at `com.dark.tool_neuron` until a follow-up rename. minSdk 29 · targetSdk 36 · abiFilters `arm64-v8a`, `x86_64`.
...
- **Dev (debug):** `cd /home/leo/data/Projects/friday-android/Sources/app && ./gradlew :app:compileDebugKotlin` to verify compilation. `./gradlew :app:installDebug` to install. Never `assemble + adb install`. Never invoke Gradle from `Sources/` directly — the Gradle root is `Sources/app/`.
...
1. **HXS-only persisted storage.** No `SharedPreferences`, Room, DataStore, or raw files. The only exception is `app_bootstrap/k.bin` (XOR-masked raw blob holding the Keystore-wrapped DEK) — it has to live outside the encrypted vault by construction. The backend at `Sources/api` does NOT receive prompt, context, transcript, or assistant response content by default — only auth, profile, push routing, policy, and health metadata. User-owned gateways are called directly from Android; there is no FRIDAY API proxy mode in this phase, and the FRIDAY-hosted provider is a disabled placeholder for a later phase.
...
5. **ViewModels live under `com.dark.tool_neuron.viewmodel` (current source tree; will move under the `com.friday.ai` package on the M1 source-rename follow-up).** Never co-locate a VM with its screen.
...
`RECORD_AUDIO` requested at first mic tap via `ActivityResultContracts.RequestPermission`; on grant, immediately `startRecording()`. The chat-side STT path still holds the UI while recording and uses no foreground service. (M2-04 added `FOREGROUND_SERVICE_MICROPHONE` + a mic-type `VoiceSessionForegroundService` for the **Friday live-voice** opt-in background-continuation only — see `## M2-04`.)
...
The service is opt-in: requires `BIND_ACCESSIBILITY_SERVICE` (system-granted) which the user enables via Settings → Accessibility → FRIDAY AI Island. The prototype screen surfaces a button that deep-links to `Settings.ACTION_ACCESSIBILITY_SETTINGS`. Detection of enabled state uses both `IslandPositionStore.accessibilityActive` (live, set in `onServiceConnected`/`onUnbind`) AND `Settings.Secure.ENABLED_ACCESSIBILITY_SERVICES` parsing (initial render before the service connects).
...
Sequence: Intro → **FridayLogin** (if !hasJwt) → **LanguageSelection** (if !languageFirstSelectionDone, M2-00) → **TermsConditions** (if !tcAccepted) → **DevNotes** (if !onboardingComplete) → SetupScreen (lock mode) → (SetupPassword if password chosen) → SetupTheme → ModelSetup → **FridayVoice**. `SetupRag` is NOT part of the mandatory first-run path (removed 2026-07-09, FRI-573) — after `ModelSetup` the flow lands directly on the Friday voice home. The `SetupRagScreen` composable + route still exist for optional/settings reach but no onboarding gate routes to it.

`ScaffoldViewModel.resolveStartDestination()` ordering: **Friday JWT gate first** (`!accountRepository.hasJwt → FridayLogin`), then **language latch** (`!languageController.isFirstSelectionDone() → LanguageSelection`), then tcAccepted, then onboardingComplete, then securitySetupDone, then modelSetupDone, then `isLockEnabled` → PasswordScreen, else **FridayVoice** (the post-auth landing is the Friday voice home, not the legacy ToolNeuron HomeScreen). See `### M1-02 Friday design shell` below.

**Fresh-resolving next-destination (FRI-573).** The three mid-session gates (`IntroScreen` finish, `FridayLogin` post-auth, `LanguageSelection` continue) navigate via `resolveNext()` — a lambda that re-runs `resolveStartDestination()` at click time — NOT the `nextDestination` value captured by `remember { … }` at first composition. On a fresh unauthenticated cold start, that captured value is `FridayLogin`; using it after Google login would jump straight into FridayLogin/FridayVoice and skip LanguageSelection → onboarding. `resolveNext()` re-evaluates `hasJwt` + the language latch so post-login routing enters the full onboarding chain. Don't revert these three sites back to the static `nextDestination`.
...
- Don't break the setup-flow handoff. Order is FridayLogin (if !hasJwt) → LanguageSelection (if !languageFirstSelectionDone) → TermsConditions (if !tcAccepted) → DevNotes (if !onboardingComplete) → SetupScreen → SetupTheme → ModelSetup → FridayVoice. `ScaffoldViewModel.resolveStartDestination()` checks `hasJwt` FIRST, then `languageController.isFirstSelectionDone()`, then `tcAccepted`, then `onboardingComplete`, then security/model gates. AppScaffold callback chain: `onTermsAccepted → DevNotes`, `onSetupComplete → SetupTheme`, `onThemeSetupComplete → ModelSetup`, `onModelSetupComplete → markModelSetupDone + FridayVoice`. `SetupRag` is NOT in the mandatory path (FRI-573). T&C must come before DevNotes — DevNotes is informational; T&C is the user's legal acknowledgment. Re-ordering them is a regression.
- Don't route the Intro / FridayLogin-post-auth / LanguageSelection gates off a `remember`'d start-destination value. That value is captured at first composition (when a fresh install has no JWT → `FridayLogin`) and goes stale the moment login flips `hasJwt`. These three dynamic gates in `TNavigation` MUST call `resolveNext()` (a lambda over `scaffoldViewModel.resolveStartDestination()`), not the captured `nextDestination`, or a fresh Google login jumps straight to FridayVoice and bypasses LanguageSelection → onboarding (FRI-573 blocker).
- Don't read an `@Inject lateinit` field inside `MainActivity.attachBaseContext()`. Hilt Activity field injection is not guaranteed to run before `attachBaseContext`, so it risks `UninitializedPropertyAccessException` on cold start. Resolve `AppCompatLocaleWrapper` via the `LocaleEntryPoint` Hilt entry point (`EntryPointAccessors.fromApplication(newBase.applicationContext, ...)`) — the SingletonComponent is ready as soon as the `@HiltAndroidApp` Application exists (FRI-573 blocker).
...
- Don't auto-request `FOREGROUND_SERVICE_MICROPHONE` for the chat-side STT path. It stays UI-held with no service. The permission + mic-type `VoiceSessionForegroundService` exist ONLY for the Friday live-voice opt-in continuation (`friday_voice_foreground_continue`, default off) and the service starts only at the session-start foreground moment (Android 14+ mic-FGS start rule) — see `## M2-04`.
...
All accept `innerPadding: PaddingValues`, no inner Scaffold, carry their own internal bars per design. `FridaySplashScreen` (2.4s timer → login/voice), `FridayLoginScreen` (Google-only: the username/password fields were removed entirely — the screen is logo/title/subtitle + one vertically-centered "Continue with Google" button + a terms/privacy footer. No Facebook/register/LoginNow path — satisfies "no fake login". Sign-in failures are surfaced by typed `SignInError`: `NoCredential` → centered `AlertDialog` ("Chưa có tài khoản Google" + primary "Mở cài đặt Google" → `Settings.ACTION_ADD_ACCOUNT` w/ `EXTRA_ACCOUNT_TYPES=["com.google"]`, fallback `ACTION_SYNC_SETTINGS`→`ACTION_SETTINGS`; never auto-opens Settings) + dismiss "Đóng"; `Cancelled`/`Other` → non-blocking `Snackbar`. Raw credential-manager strings like "[28433]" never reach the UI — `Other` logs the cause to logcat only. VM (`AccountViewModel`) exposes `noGoogleAccount: StateFlow<Boolean>` for the dialog + a `snackbar` Channel for the ephemeral messages), `FridayVoiceScreen` *(M2-04 replaced the orb/waveform with `FridayVoiceAnimation` asset playback — `FridayOrb`/`FridayWaveform` are deleted)*, `FridayChatScreen`, `FridayHistoryScreen`, `FridaySettingsScreen`, `FridayDrawerContent` (FRIDAY provider disabled first row + GGUF models + appearance toggle + red sign-out). Primitives in `friday/components/` (`FridayThinkingDots`, `FridayBubble`, `FridayAssets`). Brand assets `friday_logo.png` + `friday_avatar.jpg` in `assets/`; M2-04 added the 4 design animation assets (`orb.mp4` + 3 GIFs) per anh Long's explicit override of the original no-orb.mp4 decision.
...
- ~~Don't copy `orb.mp4` into `assets/`~~ — **superseded by M2-04 (FRI-562)**: anh Long explicitly chose to ship all 4 design animation assets (`orb.mp4`, `voice_anim_1.gif`, `voice_anim_3.gif`, `voice_anim_4.gif`) rendered by `FridayVoiceAnimation`; the Compose `FridayOrb`/`FridayWaveform` primitives were deleted.
...
- `FridayVoiceViewModel` — *(rewritten in M2-04, see `## M2-04` — now an epoch-guarded turn machine over cloud live sessions + local STT/TTS)*. Both transcript and reply still persist to the same `friday_store_v1` vault with `viaVoice=true`.
...
- `FridayVoiceScreen` — *(gesture superseded by M2-04: mic is tap-to-start / tap-to-end now, see `## M2-04`)* Requests `RECORD_AUDIO` at first mic use via `ActivityResultContracts.RequestPermission`. Error banner; animation binds to session state.
...
- ~~Don't change the mic to tap-toggle~~ — **superseded by M2-04 (FRI-562)**: the Voice Home mic is now tap-to-start / tap-to-end (`onMicTap` drives `FridayVoiceTurnMachine`; the second tap is `EndCloudUserTurn`/`StopLocalRecordingAndRecognize`), matching the live-session design. The M1-03 hold gesture is gone.
...
- `GatewayProvider` gained `needsKey`, `urlRequired`, `isLocal`, and `roles: Set<GatewayRole>`, plus a new `LOCAL` provider (keyless/urlless, on-device, both roles). `FRIDAY` stays `enabled=false` with `roles = emptySet()`. Defaults track the design: Gemini `gemini-2.0-flash`, OpenAI `gpt-4o-mini`, Anthropic `claude-sonnet-4-5`, DeepSeek `deepseek-chat`. `urlRequired` is exactly `{OPENCLAW, HERMES, CUSTOM}`. Voice-capable = `{GEMINI, LOCAL}` (M2-04/FRI-562: OpenAI Realtime voice is **deferred** — `OPENAI.roles = {BRAIN}` only; the `LiveVoiceAdapter` seam/contract is kept so a production OpenAI Realtime adapter can add it back later); everything else enabled is brain-only. `forRole(role)` filters enabled providers by capability.
...
The single decision point for where a turn goes; brain and voice stay independent. `resolveBrain()` returns `Cloud` / `Local(model)` / `Unavailable(reason)` / `None` — resolved **per turn**, so a mid-turn provider/model switch never disturbs an in-flight stream. The decision is split into a pure `decideBrain(brain, localModel)` companion fn (unit-tested off-device) that also guards `supportsRole(BRAIN)` so a roleless FRIDAY config classifies as `None`, never `Cloud`. Cloud brains stream through `DirectGatewayClient`; Local brains stream through the on-device `InferenceClient` + first GGUF model, normalized to `GatewayEvent`. Brain bridge: `brainTurn` (chat send + voice-after-transcript), `brainContinue` (fresh turn over the same history), `brainCancel` (→ `modelSession.stopGeneration()` for local; cloud turns stop when the collecting Job is cancelled — `stream()` rethrows `CancellationException` untouched and `readSse` closes the `HttpURLConnection`; also fails any parked confirmation), `brainConfirm` (resolves a `ConfirmationGate` latch — returns `false` when nothing is awaiting; `requireConfirmation()` arms it and a turn can await it before a user-gated action). The four ops are declared on the `BrainBridge` interface (`@Binds` → `GatewayRoleRouter`) so the voice layer is testable against a fake. No Brain Gateway → `brainTurn` emits an `Error` the screen surfaces (the provider/model selector UI is FRI-582/574/583).

### Voice routing (`repo/gateway/VoiceRouter.kt`, `VoiceBridge.kt`)

`VoiceRouter.route()` resolves the active Voice Gateway per turn via a pure `decide(voice, brain)` companion fn (unit-tested). Both audio routes bridge reasoning to the brain, so a brain is **required before either can open the mic**: no brain → `NoBrain`, no voice gateway → `NoVoice`. `VoiceRoute.LOCAL` → `LocalBridge(voice, brain)`: on-device STT produces a transcript, `brainTurn` runs the reply, on-device TTS speaks it. `VoiceRoute.CLOUD` → `CloudBridge(voice, brain)`: the realtime-audio transport (Gemini Live / OpenAI Realtime bidirectional socket) is owned by FRI-548 per the FRI-530 decomposition; the transcript it yields bridges through the **same** `VoiceBridge.brainTurn` seam — the brain never sees audio, only text. `VoiceBridge.kt` is that seam and exposes all four bridge ops (`brainTurn`/`brainContinue`/`brainCancel`/`brainConfirm`) over the `BrainBridge` interface; local STT and the FRI-548 cloud transport both feed transcripts into it. `FridayVoiceViewModel` calls `voiceRouter.route()` in `startListening()` and gates the mic: `NoVoice`/`NoBrain` open the provider/model selector (`VoiceSelectorRequest`) before any recording.

### VM wiring

- `FridayChatViewModel` routes `send` through `router.brainTurn(history)`; a missing brain surfaces `router.brainUnavailableMessage()`.
- `FridayVoiceViewModel` first asks `voiceRouter.route()` in `startListening()` — `LocalBridge`/`CloudBridge` open the mic and capture the resolved brain for the turn; `NoVoice`/`NoBrain` emit a `VoiceSelectorRequest` (screen opens the selector) **before any recording**, never capture-then-fail. `stopListening()` bridges audio→`bridge.brainTurn`; audio handling and reasoning are split. `cancel()` cancels the flow Job, calls `bridge.cancel()` (→ `brainCancel`), then stops speaking/recording.
...
Unit (JVM, off-device, 40 tests): `GatewayValidationTest`, `GatewayErrorSanitizerTest` (proves no secret leaks), `GatewayProviderRoleTest`, `GatewayRoleRouterTest` (brain cloud/local/unavailable decision + no-FRIDAY-API guarantee), `VoiceRouterTest` (local-bridge / cloud-bridge both reach the brain / no-voice / no-brain + cloud-voice-never-implies-FRIDAY-API), `DirectGatewayClientErrorTest` (rethrows `CancellationException`, sanitizes the literal key + bearer/x-api-key in error text). Instrumented: `GatewayConfigRepositoryTest` (CRUD, restart persistence, multi-instance same provider, status persistence, sanitized-error-not-key, independent brain/voice, brain-only-never-voice, active-delete fallback, no-provider state) — **run green on a real SM-A325F (API 33), `OK (8 tests)`**.
...
- Don't swallow `CancellationException` in `DirectGatewayClient.stream()` / `probe`. Rethrow it untouched; converting CE to `GatewayEvent.Error` would surface "cancelled" as a chat error during in-flight cancellation. The same applies to `testConnection`.
- Don't open the mic before a Brain Gateway exists. Both voice routes bridge reasoning to the brain, so `VoiceRouter.decide` returns `NoBrain` when none is selected and the VM raises the selector before recording — never capture audio then fail after transcription. Cloud realtime-audio transport (the bidirectional socket) is FRI-548's scope; `CloudBridge` here is the router seam that hands the transcript to `brainTurn`.
...
- `GeminiLiveConfig.kt` — resolved wire config from a `GatewayConfig` (apiKey/model/voice/locale/bargeIn + host/port/path). `wireModel` prefixes `models/`; `from(GatewayConfig,…)` parses `baseUrl` into `baseHost`/`basePort`/`basePath` via `URI` (a blank/invalid/non-TLS endpoint falls back to the secure default), defaults model `gemini-3.1-flash-live-preview` / voice `Aoede` / port 443.
...
- `WebSocketFrame.kt` — pure RFC 6455 frame codec. `encode` masks client→server frames (§5.3, injectable rng for deterministic tests); `readFrame` decodes server frames (returns null on clean EOF) and REJECTS violations with `ProtocolException`: masked server frame, RSV bits, reserved opcode, fragmented control, control payload > 125. `parseClose` rejects a 1-byte payload, a non-wire status code (1004/1005/1006/1015/reserved ranges via `isValidReceivedCloseCode`), and an invalid-UTF-8 reason. Handles 7-bit / 16-bit / 64-bit length forms; per-frame cap `MAX_FRAME_BYTES`.
- `WebSocketHandshake.kt` — pure RFC 6455 §4 opening handshake: `nonce`, `request(host,port,path,key)` (Host authority carries a non-443 port, `Host: host:8443`), `expectedAccept` (SHA1(key+GUID)), `isValidResponse` (101 + `Upgrade: websocket` + `Connection: Upgrade` + Accept match), `statusCode`.
...
Seams (interfaces so the engine is unit-testable off-device against fakes):
- `LiveTransport.kt` — transport interface (`open(host,port,path,onReady)`/`sendText`/`close` + nested `Incoming` {Text/Binary/Closed} + `HandshakeException(httpStatus)`). The engine depends on this, not the concrete socket, so a fake can script inbound frames.
- `LiveAudioPorts.kt` — `LiveAudioSource` (mic) + `LiveAudioSink` (playback) interfaces; BOTH `start` methods take an `onError` callback so a runtime mic/speaker failure surfaces as terminal AUDIO instead of a silent session. `AudioRecord`/`AudioTrack` can't run in a JVM test, so the engine takes these seams; fakes count start/stop/flush for lifecycle assertions.

Android I/O layer (concrete impls of the seams):
- `LiveWebSocketTransport.kt` — `LiveTransport` over a **system-trust, hostname-verified `SSLSocket`** (`hardenTls` sets `endpointIdentificationAlgorithm = "HTTPS"` before `startHandshake()`). The repo has **no WebSocket dependency** (`:networking` is GET-only curl-impersonate, `HttpURLConnection` can't upgrade), so the transport is hand-rolled — no new dep, connects straight to the user's Gemini host; `connect` is injectable so loopback tests drive the real read/close path. `open(host,port,path,onReady)` returns inbound frames as a `callbackFlow` on `Dispatchers.IO`; `onReady` fires right after a successful upgrade so the caller sends the mandatory `setup` first frame. The read loop enforces the RFC 6455 fragment sequence (orphan continuation / interleaved data frame → `ProtocolException`), caps the aggregate message at 16 MiB, decodes UTF-8 once at FIN, and completes the producer on peer close/EOF. On a peer close it ECHOES the received status (1005→1000); a protocol violation closes with **1002**. Writes are `synchronized`; a write failure surfaces `Incoming.TransportError` + closes (never swallowed). The handshake head is capped at 16 KiB; rejection → `LiveTransport.HandshakeException(httpStatus)`.
- `LiveAudioCapture.kt` — `@Singleton` `LiveAudioSource`, 16kHz mono PCM16 `AudioRecord` (VOICE_RECOGNITION), 100ms (3200-byte) chunks via `onChunk`. `stop()` releases the recorder — mic never survives a turn.
- `LiveAudioPlayer.kt` — `@Singleton` `LiveAudioSink`, 24kHz PCM16 `AudioTrack` MODE_STREAM. Writes run on a dedicated single-consumer coroutine over a BOUNDED queue (`Channel(64, DROP_OLDEST)`); `flush()` bumps a generation token, DRAINS the superseded turn's queued chunks, then pause/flush/play; `enqueue(pcm,gen)` drops chunks from a superseded generation. `start(onError)` surfaces a build/play failure and `drain` surfaces `AudioTrack.write() < 0` once via `onError` → the engine ends the session as terminal AUDIO instead of stalling silent.

Orchestration:
- `LiveSessionEngine.kt` — `internal` class owning one session as a cold `run(config): Flow<LiveEvent>`. Connect→send setup on onReady→CONFIGURED on `setupComplete`→start mic + open playback→STREAMING on first audio, with `LiveRetryPolicy` bounded reconnect on transient failures (per-attempt backoff resets once an attempt reaches CONFIGURED, but a hard `MAX_TOTAL_RECONNECTS` session ceiling stops a configure/drop loop from reconnecting forever). Non-local returns from `collect` are illegal, so server/send/capture failures are captured via `AtomicReference` and close the socket; `run()` closes the `callbackFlow` producer when the retry loop ends so a terminal session completes for collectors instead of hanging on `awaitClose`. Only a clean WS close (**1000**) ends without retry (1005/other → REMOTE_CLOSE retry); a client send failure arrives as `Incoming.TransportError` and is **classified** (not a silent EOF); mic denial → terminal `PERMISSION`, a mic/speaker runtime failure → terminal `AUDIO`, both never idling into a TIMEOUT loop. A server error is sanitized with the active `apiKey` before it reaches the UI. **Lifecycle contract** (FRI-548 owns transport/session teardown; FRI-562's UI calls these): `onAudioFocusLost`/`onBackground` → stop capture + flush playback + full teardown, `onAudioRouteChanged` → flush queued playback only and keep streaming, `onPermissionRevoked` → terminal ERROR + teardown, `endUserTurn` → stop capture + `activityEnd` (manual VAD) XOR `audioStreamEnd` (automatic VAD), `cancel`/`teardown` → stop capture+playback+socket+scope (terminal ERROR never masked by CLOSED). `classifyTransport` maps socket/handshake/protocol exceptions → `LiveErrorKind`.
- `LiveBrainGateway.kt` — the typed brain seam `LiveCloudBridge` routes through. Unlike the plain `BrainBridge`, every op **carries a `BrainCorrelation(sessionId, turnId)`** — `brainTurn(correlation,history)`, `brainContinue(correlation,history)`, `brainCancel(correlation)`, `brainConfirm(correlation)` — so the identity reaches the implementation instead of living only as a local field. Only text (transcript) crosses; never audio.
- `BrainGatewayLiveAdapter.kt` — `@Singleton` production `LiveBrainGateway` adapting the correlation-carrying seam onto the app's real `BrainBridge` (`GatewayRoleRouter`). Tracks the full active `BrainCorrelation` (session + turn) under a single lock that spans the ownership check AND the identity-less brain call — a turn open can never interleave between confirm/cancel's check and its forward, so a superseded turn OR a foreign session is dropped (cross-session isolation) and a confirm can never land on a newer turn's gate. The underlying brain's cancel is turn-scoped (collecting Job / armed gate); the adapter OWNS correlation since `BrainBridge` is identity-less by contract.
- `LiveCloudBridge.kt` — the **FRI-530 CloudBridge**: routes the live transcript to the `LiveBrainGateway` seam via the four `brain_*` ops. `brainTurn(history,transcript)` appends the transcript as the new user turn + mints a fresh `BrainCorrelation` under the stable `sessionId`; `brainContinue(history)` re-runs with a fresh turnId; `brainCancel()` takes the in-flight correlation and forwards it as one locked unit; typed `cancelTurn`/`confirmTurn` share the same lock as turn-open, so the ownership check and the seam call are atomic end-to-end — a stale correlation (superseded turn / foreign session) can never clear or resolve a newer turn, even racing against a concurrent turn open. **Audio never crosses this seam — only text.**
- `LiveVoiceSessionFactory.kt` — `@Singleton`, injects `LiveAudioCapture`/`LiveAudioPlayer` (as the `LiveAudioSource`/`LiveAudioSink` seams) + the `LiveBrainGateway`. `create(GatewayConfig,voice,locale,bargeIn)` mints one `sessionId`, atomically supersedes any prior active session (cancels it) so only ONE `LiveVoiceSession` is live at a time (shared singleton mic/player can't be raced by two overlapping sessions), and returns a `LiveVoiceSession` pairing the engine (over the `LiveTransport` seam) with a `LiveCloudBridge(brain, sessionId)`; exposes `run()`/`endUserTurn()`/the lifecycle passthroughs/`cancel()` (cancels the in-flight brain turn then the engine). `supports()` gates to `GatewayWireFormat.GEMINI`. This is the injectable seam FRI-562's Voice Home calls; it owns transport/session + brain-bridge wiring, never the brain reasoning or the Android action layer.

### Tests (JVM, off-device)

`LiveProtocolTest` (17 — setup/audio/end framing, server-frame parse + interrupt-before-turnComplete ordering, malformed→Unknown, error taxonomy), `WebSocketFrameTest` (17 — mask bit, roundtrip, 7/16/64-bit lengths, fragmentation, close parse, RFC 6455 validation: masked server frame / RSV / reserved opcode / fragmented control / >125-byte control → `ProtocolException`, **parseClose rejects 1-byte payload / reserved status / invalid-UTF-8 reason**), `WebSocketHandshakeTest` (11 — Accept digest, 101 validation, status extraction, **Host authority carries a non-443 port**, **missing Upgrade/Connection headers rejected**, **RFC 6455 token-list matching: `notupgrade`/`websocketevil`/prefix-suffix impostors rejected, `keep-alive, Upgrade` + case/whitespace variants accepted**), `LiveRetryPolicyTest` (8 — only transient loops, bounded backoff, quota never loops), `GeminiLiveConfigTest` (10 — host/port/path parse from the user URL, model prefixing, **current documented live model**, endpoint path carries base path + key, non-TLS/invalid endpoint falls back to the secure Gemini default), `LiveSessionEngineClassifyTest` (10 — auth/model/quota/network mapping), `LiveJsonTest` (4 — escaping/no-secret-leak + parse), `LiveCloudBridgeTest` (11 — the four brain_* ops deliver the BrainCorrelation to a fake seam, stale/foreign cancel+confirm dropped, **confirm-vs-new-turn-open serialized end-to-end via a latch-pinned race**), `LiveSessionEngineLifecycleTest` (15 — focus loss/background teardown, route-change flush-only, permission revoke terminal, idle + real running-session rapid start/stop no-leak, mic-fails-after-open → AUDIO, network drop→recovery, auth non-retryable, collector-cancel teardown, mic-denied → PERMISSION, **configure/drop loop terminates with bounded reconnects**, **playback start failure → AUDIO**, **send failure classified NETWORK not silent**, **server error echoing the key is sanitized**), `LiveWebSocketTransportStreamTest` (10 — **the REAL transport over a loopback socket**: peer close in-order + completes, bare EOF completes, split-UTF-8 reassembly, send failure surfaces `TransportError`, `hardenTls` HTTPS endpoint id, **orphan continuation rejected**, **interleaved data frame rejected**, **fragmented aggregate overflow rejected**, **peer close echoed with same status**, **protocol violation closes 1002**), `BrainGatewayLiveAdapterTest` (9 — production adapter over the real `BrainBridge` contract: active-only forward, superseded drop, cross-session isolation, **deterministic latch-pinned races: turn-open blocked mid-confirm, cancel-vs-confirm, foreign-session confirm during active confirm**), `LiveVoiceSessionFactoryTest` (1 — **creating a second session atomically supersedes the first through the production factory seam**). All green, 0 skipped, in `./gradlew :app:testDebugUnitTest`.
...
- Don't strip the four `brain_*` ops off `LiveCloudBridge` or make the correlation local-only. Gemini Live is audio-only transport; reasoning/tools/actions go to the Brain Gateway through the `LiveBrainGateway` seam, and **every op carries a `BrainCorrelation(sessionId, turnId)`** so the identity actually reaches the implementation (`BrainGatewayLiveAdapter` over the app's `BrainBridge`), never a FRIDAY/Firebase proxy and never the audio path. `cancelTurn`/`confirmTurn` drop a stale correlation (a superseded turn, or a foreign session) so a late barge-in never cancels the wrong turn. `LiveCloudBridgeTest` captures the correlation received at the fake seam and proves turn/continue/cancel/confirm target the right turn, incl. concurrent stale-turn cases.
- Don't move transport/session teardown ownership to FRI-562. FRI-548 owns the lifecycle contract: the host wires audio-focus/route/foreground/permission callbacks to `onAudioFocusLost`/`onAudioRouteChanged`/`onBackground`/`onPermissionRevoked`, but the teardown discipline (stop capture, flush/stop playback, close socket, cancel scope) lives in `LiveSessionEngine` so the mic/socket never outlive the reason to hold them. `onAudioRouteChanged` flushes playback only and keeps the session; every other lifecycle event is a full teardown (background/focus-loss) or terminal error (permission revoke). `LiveSessionEngineLifecycleTest` drives each event against fakes and asserts no leaked mic/socket + rapid start/stop idempotence + network-drop->recovery reconnect.
- Don't remove the `run()` producer-close at the end of the retry loop. A terminal attempt (auth/model/permission, or the retry budget exhausted) must `close()` the `callbackFlow` so a `toList()`/finite collector completes; without it the flow hangs on `awaitClose` forever after the loop breaks.
- Don't scatter the live lifecycle teardown into the UI. FRI-548 owns transport/session teardown; the host (FRI-562 Voice Home) only wires OS callbacks to `LiveVoiceSession`/`LiveSessionEngine` entry points: `onAudioFocusLost()` and `onBackground()` = stop capture + flush playback + full teardown (never hold the mic off-screen); `onAudioRouteChanged()` (headset/BT/speaker) = flush queued playback only, session stays alive; `onPermissionRevoked()` = stop capture + terminal ERROR + teardown (not a retry). Mic-open failure after `setupComplete` returns `PERMISSION` (terminal), never idles the socket into a `TIMEOUT` retry loop. A transient drop reconnects with bounded backoff; reaching `CONFIGURED` resets the attempt budget.
- Don't leave `LiveSessionEngine.run()`'s `callbackFlow` un-closed when the retry loop ends. It `close()`s the producer channel after the loop breaks (terminal ERROR / clean close / cancel) so a collector using `toList()` completes instead of hanging on `awaitClose`. The audio/transport seams (`LiveAudioSource`/`LiveAudioSink`/`LiveTransport`) exist so the engine's connect→configure→stream→teardown flow and all lifecycle transitions are unit-tested off-device against fakes — don't collapse them back into the concrete Android classes.
- Don't let `LiveWebSocketTransport.open()` fall into `awaitClose` without completing the producer channel on a normal peer close / EOF. The read loop breaks on `OPCODE_CLOSE` or `readFrame == null` (EOF); the flow MUST then call the unqualified `close()` (ProducerScope.close) so `LiveSessionEngine.connectOnce`'s `collect` returns instead of suspending forever. This bug shipped once (the fake transport closed its channel but the real one didn't), so it is now covered by `LiveWebSocketTransportStreamTest` exercising the REAL transport over a loopback socket for both close paths. The connector is injectable (`connect: (host,port)->Socket`, default = system-trust TLS) precisely so that test can drive the real read loop without TLS — keep it injectable.
- Don't swallow a mic runtime failure that happens AFTER a successful open. `LiveAudioSource.start` takes `onError`; `LiveAudioCapture` fires it once (guarded by `recording.compareAndSet(true,false)` so a deliberate `stop()` stays silent) when `rec.read()` returns `<0` or the recorder throws. `LiveSessionEngine.startMicStreaming` records `LiveErrorKind.AUDIO` + closes the socket so `connectOnce` ends the attempt as terminal AUDIO (never a retry loop, never a session silently running on a dead mic). `micFailsAfterOpen_surfacesAudioError_notRetryLoop` drives this through the public engine path. `AUDIO` is NOT in `LiveErrorKind.transient` — don't add it.
- Don't reduce `BrainGatewayLiveAdapter`'s tracked identity to just `turnId`. The adapter is a `@Singleton`, so a same-`turnId` correlation from a different session would false-match a `turnId`-only guard. It tracks the full `BrainCorrelation` (session + turn) and forwards cancel/confirm only on a full match — that is the cross-session isolation `BrainGatewayLiveAdapterTest.crossSession_sameTurnIdDifferentSession_isDropped` proves. The underlying `BrainBridge` is identity-less by contract; the adapter OWNS correlation, and that ownership + its production tests are the approved contract (don't quietly push identity into `BrainBridge` without updating both).
- Don't replace the correlation locks with check-then-call on an `AtomicReference`. Both `BrainGatewayLiveAdapter` and `LiveCloudBridge` hold ONE lock across {ownership check + identity-less brain/seam call} for turn-open, cancel, and confirm. A lock-free `get()==` check followed by the call reintroduces the confirm race: A's check passes, B's turn opens and arms its gate, then A's identity-less `brainConfirm()` resolves B's gate. The latch-pinned race tests (`concurrentTurnOpen_cannotInterleaveInsideConfirm`, `concurrentCancelAndConfirm_onlyCancelReachesBrain`, `foreignSessionConfirm_duringActiveConfirm_neverReachesBrain`, `confirmTurn_vsNewTurnOpen_isSerializedNotInterleaved`) exist to kill that regression — they block inside the brain call and prove a racing open waits. The brain calls under the lock are bounded (Flow construction is cold; `ConfirmationGate.confirm()`/`cancel()` are latch completes; `stopGeneration` is a fast synchronous binder call whose handler only sets the engine stop flag and swallows exceptions), so the lock hold is short — and moving the cancel outside the lock would reintroduce the race.
- Don't drop TLS hostname verification. `LiveWebSocketTransport.hardenTls` sets `SSLParameters.endpointIdentificationAlgorithm = "HTTPS"` (RFC 2818) before `startHandshake()`. A raw `SSLSocket` validates the cert CHAIN but NOT that the cert belongs to the host — without this, the API key (in the URL), audio, and transcript are open to MITM. `classifyTransport` maps `SSLPeerUnverifiedException` → `AUTH`. `hardenTls_enablesHttpsEndpointIdentification` guards it.
- Don't swallow send failures in the transport. `sendText`/`writeControl` go through `writeFrame`, which on any write throwable records `writeError` and `close()`s the transport so the read loop ends and the engine classifies the drop — a silently discarded `runCatching` on setup/audio writes is the regression. `sendFailureAfterHandshake_isSurfacedNotSwallowed` proves it via a fake socket that fails the post-handshake write.
- Don't revert `GeminiLiveConfig.DEFAULT_MODEL` to `gemini-2.0-flash-live-001` (shut down 2025-12-09) or `gemini-2.5-flash-native-audio-preview-12-2025` (superseded). The default is `gemini-3.1-flash-live-preview`, the current documented Gemini Live model — ai.google.dev's model page explicitly instructs migrating the model string from `gemini-2.5-flash-native-audio-preview-12-2025` to it. Our setup payload stays compatible: we send no `thinkingBudget` (3.1 uses `thinkingLevel`, default applies), and `parseServerFrame` already walks ALL `modelTurn.parts` (3.1 can emit multiple content parts per `serverContent`). Bump only to a model on the official live-model list when Google rotates again; a dead default means the out-of-box voice path can't connect.
- Don't parse the user's custom endpoint as host-only. `GeminiLiveConfig.parseEndpoint` resolves host/port/path from the base URL via `URI`, and `endpointPath` prepends `basePath`; a blank/invalid or non-TLS (`http`/`ws`) endpoint falls back to the secure Gemini default rather than downgrading the socket to cleartext. `basePort` (default 443) is honored on connect. Don't hardcode 443/empty-path back into the engine.
- Don't send both `activityEnd` and `audioStreamEnd` on turn end. `endUserTurn` sends `activityEnd` when barge-in is off (manual VAD owns the boundary) and `audioStreamEnd` when barge-in is on (automatic VAD) — one or the other, never both. Sending both is the wrong manual-VAD sequence.
- Don't make the `LiveAudioPlayer` queue unbounded. It's `Channel(QUEUE_CAPACITY=64, onBufferOverflow = DROP_OLDEST)` so a slow `AudioTrack` can't grow RAM without limit (newest audio wins). `flush()` also drains the queued stale chunks (`tryReceive` until empty) before pausing/flushing the track, so a barge-in doesn't leave superseded PCM to delay the next turn.
- Don't validate server frames loosely. `WebSocketFrame.readFrame` rejects RFC 6455 violations with `ProtocolException`: RSV bits set, reserved opcode, a masked server frame (§5.1), a fragmented control frame, and a control payload > 125 bytes (§5.5). The engine maps `ProtocolException` → `PROTOCOL` (terminal). Don't relax these into silent passes.
- Don't treat WebSocket close 1005 as a clean success. Only code 1000 is a deliberate clean end; 1005 (no-status sentinel, never a wire code) and any other close are abnormal drops worth retrying (`PeerClose.isClean` is `code == 1000` only, RFC 6455 §7.4).
- Don't drop the port from the Host authority. `WebSocketHandshake.request(host, port, path, key)` emits `Host: host:port` for a non-443 port (omitted only for 443) — a custom `https://proxy:8443` endpoint must ask for the right virtual host or the proxy serves the wrong site.
- Don't relax the fragment state machine or its aggregate cap in `LiveWebSocketTransport.readLoop`. An orphan `CONTINUATION`, a new TEXT/BINARY while a fragmented message is open, and an aggregate over `MAX_MESSAGE_BYTES` (16 MiB) are `ProtocolException`s — quietly resetting the buffer on interleave was the bug that let a peer corrupt message assembly, and an uncapped aggregate is an OOM vector.
- Don't turn a send failure back into a silent close. `writeFrame` failure emits `Incoming.TransportError(cause)` on the open flow AND closes; `connectOnce` classifies the cause (`sendFailure`) so the engine reports the real fault instead of a generic EOF/REMOTE_CLOSE. The transport's post-write-failure read exception is deliberately NOT rethrown (the TransportError already carries the cause).
- Don't let two `LiveVoiceSession`s run concurrently. The mic/player/brain are app singletons; `LiveVoiceSessionFactory.create()` atomically supersedes (cancels) the prior session via `AtomicReference.getAndSet`. Removing that supersession lets overlapping sessions share capture callbacks and cross-cancel each other's teardown.
- Don't skip the `Upgrade`/`Connection` header checks in `WebSocketHandshake.isValidResponse`, and don't uncap `readHandshakeResponse` (16 KiB head max) — a 101 without a real upgrade or an unbounded head are both hostile-peer vectors.
- Don't cap-reset the reconnect loop into infinity. The per-attempt backoff reset on CONFIGURED stays, but `MAX_TOTAL_RECONNECTS` (10) is a hard session ceiling — a peer that configures-then-drops repeatedly must reach terminal ERROR, not spin the radio forever (`repeatedConfigureThenDrop_terminatesWithBoundedReconnects`).
- Don't sanitize a server-reported error without the active key. `handleServerText`'s `Failure` branch calls `GatewayErrorSanitizer.sanitize(frame.message, config.apiKey)` — a provider echoing the exact key in its error body must be redacted before the message reaches `LiveEvent.Error` (`serverErrorEchoingKey_isSanitized`).
- Don't reply to a peer close with a fixed 1000 or to a protocol violation with a success close. The transport echoes the received valid status (1005 no-status → 1000) and closes with **1002** on a `ProtocolException` — wire-level asserted by `peerClose_isEchoedWithSameStatus` and `protocolViolation_closesWith1002`.

---

## M2-04 Live voice orchestration, default assistant, inbound event UI (FRI-562)

Binds the FRI-548 Gemini Live session engine, FRI-553 role router, and FRI-557 ContextEngine into the real Voice Home: a full tap-to-start / tap-to-end live-voice turn loop, an Android default-assistant entry point (long-press home), and the inbound event render path. Everything stays HXS-local / direct-gateway; no FRIDAY API proxy, no Firebase touch on transcript/key/audio.

### Turn machine (`viewmodel/FridayVoiceTurnMachine.kt`)

Pure `(VoiceTurnState, VoiceTurnEvent) → VoiceTurnResult(state, effects)` reducer — no Android, fully JVM-tested (`FridayVoiceTurnMachineTest`). `VoiceUiState` is the 9-state UI surface (Idle / RequestingPermission / Connecting / Listening / Thinking / Speaking / Done / Cancelling / Error(sanitized msg)). **Epoch guard**: every async-arriving event (`SessionEvent`, `LocalTranscript`, `BrainDelta/Done/Error`, `TeardownComplete`) carries the epoch it was launched under; a mismatched epoch is dropped verbatim — no UI mutation, no effects. Epoch bumps on each new turn (fresh start or barge-in), and the VM clears `question`/`answer` on epoch change so stale text never leaks into a new turn. Effects (`OpenCloudSession`, `EndCloudUserTurn`, `StartLocalRecording`, `StopLocalRecordingAndRecognize`, `RunBrainTurn`, `SpeakLocal`, `CancelBrainTurn`, `ReleaseSession`, `TeardownAll`, …) are executed by the VM outside the `synchronized` reduce block. **`ReleaseSession` vs `TeardownAll`**: `ReleaseSession` fires on every *terminal* transition (`BrainError`, session `CLOSED`, `LiveEvent.Error`, `TurnComplete`, cloud `BrainDone`, cloud barge-in-off finish, reset-from-Done/Error) — it frees the live-session resources (cloud handle, session+brain-turn jobs, FGS, lifecycle host, mic) but leaves the `question`/`answer` text and the Done/Error UI intact so the user keeps reading the result while the mic indicator goes off. `TeardownAll` is the *mid-turn cancel* path (`ResetTap` while active): `releaseSession()` **plus** stop local TTS + clear text + `TeardownComplete`. LOCAL `BrainDone` deliberately emits **only** `SpeakLocal` (no `ReleaseSession`) so local TTS keeps playing and focus-loss→reset stays armed until the user acts.

### ViewModel (`viewmodel/FridayVoiceViewModel.kt`, rewritten)

Seams for JVM testability, all Hilt-bound with Android impls: `VoiceRoutePort` (→ `VoiceRouter`), `VoiceIo` (→ `VoiceModelManager`), `ContextHistorySource` (→ `ContextEngine`), `LiveVoiceAdapter` (→ `GeminiLiveVoiceAdapter`), `VoicePrefsPort`, `VoiceGatewayStatePort`, `VoiceForegroundServicePort`, plus `VoiceSessionLifecycleHost`, `VoiceSessionServiceGate`, `PendingAssistInvocation`. The `internal` ctor takes the seams (trailing-defaulted `pendingAssist` preserves positional test call sites); the `@Inject` ctor adapts the real types.

- `onMicTap()` from Idle/Done/Error resolves the route **first** (`VoiceRouter.route()`): `NoVoice`/`NoBrain` → `VoiceSelectorRequest` (selector opens, mic never records); `LocalBridge`/`CloudBridge` → `MicTap` event. Mid-turn tap ends the user turn (cloud: `endUserTurn`; local: stop+recognize) or barge-ins per `friday_voice_barge_in`.
- Cloud route: `adapter.supports(config)` gate (unsupported → honest Error, session never opens), `adapter.open(config, DEFAULT_VOICE, locale, bargeIn)` → `LiveVoiceHandle`; `run()` events map into the machine; `InputTranscript` → `question`, `OutputTranscript` → `answer` (epoch-guarded). Brain turns go through `handle.brainTurn(history, transcript)` (the FRI-548 `LiveCloudBridge` correlation seam).
- Local route: `VoiceIo.startRecording` → `stopRecordingAndRecognize` (sherpa STT) → `RunBrainTurn` → `bridge.runTurn` → `SpeakLocal` (sherpa TTS). Failed TTS after Done flips to Error deliberately — no fake success.
- Persistence unchanged: user+assistant turns → `friday_store_v1` with `viaVoice=true`; history via `ContextEngine.buildHistory` (never raw transcript).
- `confirm()` routes to `handle.brainConfirm()` when a cloud session owns the turn, else `bridge.brainConfirm()`. `reset()` = `ResetTap` → `TeardownAll`.
- `runBrainTurn` stores its collect coroutine in `brainTurnJob`; `CancelBrainTurn` and `releaseSession()` cancel it so a superseded/terminated turn stops streaming. The `GatewayEvent.Done` persistence block is `epochCurrent(epoch)`-guarded — a cancelled/stale turn never persists an assistant reply.
- Cloud brain-turn history: `LiveCloudBridge.brainTurn` appends `GatewayTurn("user", transcript)` itself, so the VM drops the trailing duplicate (`history.dropLast(1)` when the last built-history turn equals the transcript) before `handle.brainTurn` — exactly one user turn reaches the brain. Local route (`bridge.runTurn`) passes history verbatim.
- `releaseSession()` (terminal): cancel `brainTurnJob`+`sessionJob`, `handle.cancel()` (which already cancels the in-flight brain turn — don't double-cancel) + null it, `bridge.brainCancel()`, cancel recording, detach lifecycle host, `serviceGate.setSessionActive(false)`, stop FGS. Does **not** stop TTS or clear text. `teardownAll()` = `releaseSession()` + `stopSpeaking()` + clear text + `TeardownComplete`. `onCleared()` = `releaseSession()`'s resource cancels + `stopSpeaking()`.

### Live-voice adapter seam (`repo/gateway/live/LiveVoiceAdapter.kt`, `GeminiLiveVoiceAdapter.kt`)

Provider-agnostic `LiveVoiceAdapter { supports(config); open(config, voice, locale, bargeIn): LiveVoiceHandle }` + `LiveVoiceHandle` mirroring `LiveVoiceSession` 1:1 (run/endUserTurn/lifecycle passthroughs/cancel + brainTurn/brainCancel/brainConfirm) so the VM never imports a provider type. `GeminiLiveVoiceAdapter` is a thin `@Singleton` delegate to `LiveVoiceSessionFactory` — owns nothing FRI-548 already owns. **OpenAI Realtime is contract-only** (per anh Long): a future transport implements the same interface; `LiveVoiceAdapterContractTest` pins the seam shape. Hilt: `GatewayModule` `@Binds LiveVoiceAdapter → GeminiLiveVoiceAdapter`.

### Session lifecycle host (`voice/VoiceSessionLifecycleHost.kt`, `voice/VoiceSessionServiceGate.kt`)

`VoiceSessionLifecycleHost` (`@Singleton`) registers audio-focus / audio-route (`AudioDeviceCallback`) / `ProcessLifecycleOwner` observers when a session attaches and forwards signals to the `AttachedSession` seam — **forward-only**; teardown discipline stays where FRI-548 put it. Cloud route forwards to the live handle (`onAudioFocusLost`/`onAudioRouteChanged`/`onBackground`); local route maps focus-loss/background to `reset()` (no handle exists). Attach/detach idempotent. `SystemHooks` internal interface keeps it JVM-testable (`FridayVoiceLifecycleHostTest`). When `friday_voice_foreground_continue` is on, background does NOT tear down the cloud session (continuation). `VoiceSessionServiceGate` is the Android-free service↔VM bridge: service observes `sessionActive` to stopSelf; VM collects `stopRequests` (notification Stop action) → `reset()`.

### Foreground continuation (`service/voice/VoiceSessionForegroundService.kt`)

Opt-in (`friday_voice_foreground_continue`, default **false**) mic-type FGS in the main `:app` process (session state is in-process singletons). Started only at session-start — the foreground moment — to satisfy Android 14+ mic-FGS start-time rules. Never sticky; notification carries a Stop action wired through the gate. Manifest: `FOREGROUND_SERVICE_MICROPHONE` permission + `foregroundServiceType="microphone"`.

### Default assistant (`service/assistant/`)

- `FridayVoiceInteractionService` — minimal `VoiceInteractionService` binding target (manifest: `BIND_VOICE_INTERACTION` + `@xml/interaction_service` metadata). Lists Friday in Settings → Default apps → Digital assistant.
- `FridayVoiceInteractionSessionService` → `FridayVoiceInteractionSession`: `onShow` **never renders assist UI and never touches the mic** — it sets `PendingAssistInvocation.set()` **in-process** (via a Hilt `EntryPointAccessors.fromApplication` — the session runs in the same `:app` process), then fires `startAssistantActivity(Intent(MainActivity, ACTION_ASSIST_VOICE))` (action only, **no extras**) and `hide()`s immediately. Trust boundary: the exported `MainActivity` reads **no** assist extras, so a third-party caller can't forge an assist invocation by launching the activity with a crafted extra — only the same-APK session service can raise the flag.
- `FridaySpeechRecognitionService` — honest `RecognitionService` backed by sherpa STT (`VoiceModelManager.recognizeSamples`, injected `Lazy`). **Confused-deputy guard**: rejects a caller that doesn't itself hold `RECORD_AUDIO` (`callback.callingUid` + `checkPermission(RECORD_AUDIO, -1, uid)`) before its own self-permission check — we never record on an unprivileged client's behalf with our permission. Owns its **own** `AudioRecord` (VOICE_RECOGNITION 16 kHz mono float) — never the singleton `SttRecorder`, which a live session may hold. Real `SpeechRecognizer.ERROR_*` codes: no permission → `ERROR_INSUFFICIENT_PERMISSIONS`, no STT model → `ERROR_LANGUAGE_NOT_SUPPORTED`, capture failure → `ERROR_AUDIO`, empty result → `ERROR_NO_MATCH`. All Binder callbacks (`readyForSpeech`/`beginningOfSpeech`/`endOfSpeech`/`results`/`error`) are `RemoteException`-wrapped; `finish()` is `@Synchronized` and `onCancel` shares the same monitor so cap-job expiry / `stopListening` / cancel can't race the recorder drain. 15 s utterance hard cap. No always-on hotword ("Hey Friday" is out of scope — explicit).
- Invocation flow: session sets the flag in-process → `startAssistantActivity` → `MainActivity` just resumes on `ACTION_ASSIST_VOICE` → `PendingAssistInvocation` (`@Singleton` one-shot flag, survives the lock/auth chain) → Voice screen `LaunchedEffect(pendingAssist, micGranted)` consumes exactly once → `onAssistantInvocation()` = `onMicTap()` if mic already granted, else `onMicNeedsPermission()` (assist is permission-gated so a mic-FGS start on Android 14+ never happens without while-in-use `RECORD_AUDIO`). The assist path opens the **same** turn a mic tap does, no separate audio path.
- Settings deep-link: `FridaySettingsScreen` "Set FRIDAY as your assistant" card → `ACTION_VOICE_INPUT_SETTINGS` → assist-gesture settings → `ACTION_SETTINGS` fallback chain, each try/catch.

### Inbound event render path (`repo/InboundEventCenter.kt`, seam for FRI-555)

- `model/friday/InboundEvent.kt` — `(eventId, sourceType PUSH|LOCAL, correlationId?, kind TASK|STATUS|CONFIRMATION, title, body, urgency NORMAL|HIGH|URGENT, receivedAt)`; `requiresConfirmation` ⇔ kind == CONFIRMATION.
- `InboundEventPort { publish(event) }` is the producer seam; `InboundEventCenter` (`@Singleton`, `@Binds` in `InboundEventModule`) is the consumer. Decision: app foreground → in-app `activeEvent` card (never also a notification — no double-surface); background → notification on the urgency channel (`friday_events_normal` IMPORTANCE_DEFAULT / `friday_events_high` HIGH / `friday_events_urgent` HIGH + vibration; **no full-screen intent**). Internal `ForegroundSignal`/`EventNotifier` seams keep the decision logic JVM-tested.
- **Unverified-push conservatism** (`coercePush`): any PUSH-sourced CONFIRMATION is downgraded to STATUS (unverified push must never render confirm UI), title/body capped at 120/400. `fromPushData` drops payloads missing eventId/title and defaults unknown kind/urgency to STATUS/NORMAL. `FridayMessagingService` is the only producer today (malformed → logged as dropped, **no content logging**); FRI-555 verified pushes land on the same port later. Notification tap → `MainActivity` `ACTION_OPEN_EVENT` + eventId/correlation extras (carried for FRI-555).
- UI: `InboundEventCard` (title/body/Dismiss) via `FridayEventsViewModel`; separate `ConfirmationCard` (Confirm/Cancel) is **brain-gate driven** — renders on `awaitingConfirmationState` (reactive mirror on `VoiceBridge`: the router's `awaitingConfirmation` StateFlow, constant false for non-router brains). User tap is the ONLY resolver: Confirm → `viewModel.confirm()` → `brainConfirm`; Cancel → `reset()` → `brainCancel`. No timeout auto-confirm, no auto-execute path (`InboundEventConfirmationTest`).

### New HXS prefs (encrypted `app_prefs`)

`friday_voice_anim` (String, default `"orb"`; keys `orb|gif_1|gif_3|gif_4` → `VoiceAnimation`), `friday_voice_foreground_continue` (Boolean, default false), `friday_voice_barge_in` (Boolean, default true).

### UI

`FridayVoiceScreen` rebuilt: header (`FridayVoiceHeader`), state title, `FridayVoiceRing` + `FridayVoiceAnimation` (TextureView `MediaPlayer` loop for `orb.mp4`, `AnimatedImageDrawable` for GIFs — all 4 design assets shipped per anh Long's override), provider status pill (`FridayVoiceProviderStatus`), transcript/answer scroll area, inbound event card slot, confirmation card slot, error banner (tap to reset), selector prompt, `FridayVoiceControls` (tap-toggle mic). Permission launch is single-path via `viewModel.permissionRequest` SharedFlow (avoids double-invocation from onClick).

### Tests (JVM, all green)

`FridayVoiceTurnMachineTest` (35 — reducer: full local/cloud turns, epoch staleness, barge-in on/off, permission flows, terminal `ReleaseSession` on every cloud terminal path + its absence on local `BrainDone`, teardown), `FridayVoiceViewModelTest` (15 — selector gating never records, full local turn persists `viaVoice` turns + speaks, cloud turn through fake adapter/handle, unsupported provider → Error + no session, stale events after reset don't mutate cleared text, barge-in-off finishes without re-record, assist invocation ≡ mic tap, pending-assist one-shot, rapid double-tap opens one session, terminal error releases session resources while keeping transcript, cancelled cloud turn never persists the assistant reply, cloud history carries exactly one user turn), `FridayVoiceLifecycleHostTest`, `LiveVoiceAdapterContractTest`, `PendingAssistInvocationTest` (3), `InboundEventCenterTest` (11 — channel routing, push downgrade + caps, fromPushData mapping/drops/conservative defaults, foreground card vs background notification, push coerced before any surface), `InboundEventConfirmationTest` (4 — confirm→brainConfirm exactly once, cancel never confirms, armed gate never auto-executes, reactive mirror constant-false for fakes). FRI-562 QA rework added `GeminiTtsSynthesizerErrorTest` + `LiveVoiceBrainAnswerContractTest` (B1), `FridayEventsViewModelTest` (3) + `PendingInboundEventTest` (4) + `InboundEventCenterTest` resolve/recent cases (B2), `FridayVoiceLifecycleHostTest` focus-denied rollback (B3), `GatewayProviderRoleTest`/`VoiceRouterTest` OpenAI-voice-deferred (A1). Full suite **370 green, 0 skipped** (`:app:testDebugUnitTest`).

### FRI-562 Codex-QA rework (B1/B2/B3 + A1/A2/A3)

Six review blockers, anh Long's authoritative decisions applied:

- **B1 — Brain owns the cloud answer.** Voice Gateway (Gemini Live) is **speech I/O + turn-boundary only**; the Brain Gateway OWNS the answer. `LiveSessionEngine` carries a `brainOwnsAnswer` flag: Gemini's own answer audio/text (`modelTurn` audio + `OutputTranscript`) is **discarded** when set, and cloud `TurnComplete` is **non-terminal** (it only closes the user's input turn, not the reply). The brain's streamed text is vocalized via `LiveVoiceSynthesizer` (`repo/gateway/live/LiveVoiceSynthesizer.kt`) → `GeminiTtsSynthesizer` (`gemini-2.5-flash-preview-tts:generateContent`, base64 **PCM16 24kHz**, direct to the user's own Gemini host — no FRIDAY/Firebase proxy). The reducer gained `SpeakCloud`/`SpeakComplete` events + effect; cloud `BrainDone` schedules `SpeakCloud`, `SpeakComplete` is the cloud terminal that emits `ReleaseSession`. `GeminiTtsSynthesizerErrorTest` proves error sanitization (key never leaks); `LiveVoiceBrainAnswerContractTest` pins that Gemini's own answer never surfaces.
- **B2 — Inbound notification tap routing.** `data/PendingInboundEvent.kt` (`@Singleton` one-shot `Pair<eventId, correlationId?>` flag, mirrors `PendingAssistInvocation`). `InboundEventCenter` gained a bounded access-order LRU `recent` (cap `RECENT_CAP = 32`, retains every published event foreground+background under `synchronized(recent)`), `resolve(eventId, correlationId): InboundEvent?` (correlation checked only when both non-null; unknown id → null), and `surface(event)`. `MainActivity.handleEventIntent` (onCreate after super + onNewIntent) reads `ACTION_OPEN_EVENT` + `EXTRA_OPEN_EVENT_ID`/`_CORRELATION` and sets the pending flag. `FridayEventsViewModel` collects `pendingInboundEvent.pending`, consumes once, `resolve()`→`surface()` — a background-published event surfaces its card only when the user taps the notification (never auto-surfaced). `FridayEventsViewModelTest` (3) + `PendingInboundEventTest` (4) + `InboundEventCenterTest` resolve/recent cases. **Superseded by C3 below:** the RAM-only `recent` LRU could not survive process death, so `PendingInboundEvent` now carries the full `InboundEvent` (not a `Pair`), the notification intent carries the full payload, and `surfaceFromNotification` reconstructs + re-coerces on a cold start — see the round-2 C3 entry.
- **B3 — Audio-focus failure is honest.** `VoiceSessionLifecycleHost.attach(session): Boolean` — requests focus FIRST via `SystemHooks.registerFocus` (returns grant bool); on denial it rolls back (`detach()`) and returns false **before** registering route/lifecycle listeners, so a denied focus never leaves a dangling listener or a held mic. The VM (cloud `openCloudSession` + `startLocalRecording`) checks the result: false → cancel the just-opened handle / cancel recording, `BrainError(FOCUS_DENIED_MESSAGE)`, and **never** start the FGS. `FridayVoiceLifecycleHostTest` covers focus-denied rollback + granted path.
- **A1 — OpenAI voice deferred (keep the seam).** `GatewayProvider.OPENAI.roles = {BRAIN}` only — OpenAI is a Brain Gateway, not a Voice Gateway, until a production OpenAI Realtime adapter ships. The `LiveVoiceAdapter`/`LiveVoiceHandle` seam + `LiveVoiceAdapterContractTest` are kept so adding it back is one adapter impl. `Voice-capable = {GEMINI, LOCAL}`. `GatewayProviderRoleTest`/`VoiceRouterTest` assert OpenAI is voice-excluded; downstream `VoiceRouter.decide` only branches on `voiceRoute` (capability filtering is upstream via `supportsRole(VOICE)`), so OpenAI voice configs are unreachable.
- **A2 — Voice Home UI parity.** Provider status pill moved from below-orb into the **header ROW center** (`FridayVoiceHeader` now takes `brainConfigured`/`gatewayLabel`/`ready`/`onProviderClick`, renders `FridayVoiceProviderStatusPill` weighted between menu + chat icons). `FridayVoiceProviderStatusPill` gained a `modifier` param (for `weight(1f)`). `FridayVoiceControls` LEFT button re-icon'd to the typed-chat glyph (`TnIcons.MessageCircle`) + rewired `onEditClick`→`onToChat` (routes to typed chat, not the drawer). Header LEFT menu still opens the drawer. Pure composition move; no VM/route change.
- **A3 — Thinking visual.** `FridayThinkingRow` (`ui/screens/friday/components/FridayVoiceThinkingRow.kt`): `loading.gif` @30dp + `friday_voice_thinking_label`, rendered only while `uiState is VoiceUiState.Thinking`. GIF decode is DRY — the `AnimatedImageDrawable` path was extracted from `FridayVoiceAnimation` into a shared `AssetGif(assetName, active, modifier)` helper that both call.

Device screenshot evidence (phase 14: EN/VI × light/dark × 4 accents, loading.gif animating live) is **deferred** — pending anh Long providing sign-in + a device. FRI-555 (verified inbound confirmation) remains the upstream dependency: the PUSH CONFIRMATION→STATUS downgrade stays until verified pushes land on the same `InboundEventPort`.

### FRI-562 Codex-QA rework round 2 (C1/C2/C3)

Three correctness blockers from the REOPEN_FULL_REVIEW pass:

- **C1 — Cloud brain answer must finish playing before the turn ends, and TTS failures must be honest.** The cloud brain-owns-answer path previously enqueued TTS PCM non-blocking (`sink.enqueue` returns instantly), then `speakCloud` immediately fired `SpeakComplete → ReleaseSession → handle.cancel() → sink.flush()`, cutting audio before it played. Fix: `LiveAudioSink.playToCompletion(pcm, gen): Boolean` (in `LiveAudioPlayer`) writes `WRITE_BLOCKING` then polls `playbackHeadPosition` until the written frames drain, returning false if a `flush()` bumps the generation (barge-in supersedes) or the track dies. `LiveVoiceSession.speak` captures the generation BEFORE synthesis, so a barge-in during the network call returns `Superseded`. Nullable synth/speak returns are replaced by typed results: `LiveVoiceSynthesizer.synthesize` → `SynthResult { Audio(pcm) | Empty | Failed(message) }` (`GeminiTtsSynthesizer` maps blank→Empty, HTTP non-2xx→`Failed(sanitized)`, decode-null→Failed, throw→`Failed(sanitized)`, rethrows `CancellationException`); `LiveVoiceHandle.speak` → `SpeakOutcome { Completed | Superseded | Empty | Failed(message) }`. The VM's `speakCloud` fires `SpeakComplete` only on `Completed`/`Empty`, `BrainError(msg)` on `Failed` (real TTS failure surfaces as Error, never swallowed), and drops `Superseded` (a newer turn owns the sink). `LiveVoiceBrainAnswerContractTest` records `playToCompletion` distinctly and asserts Completed/Empty/Failed outcomes.
- **C2 — Cloud barge-in must re-open the mic on the SAME socket, and the resumed transcript must pass the epoch guard.** `endUserTurn()` calls `capture.stop()` when a turn begins, so a cloud barge-in that only emitted `CancelBrainTurn` left the mic dead — the user could never speak again. Fix: `LiveSessionEngine.resumeUserTurn(): Boolean` (exposed through `LiveVoiceSession`/`LiveVoiceHandle`/`GeminiLiveVoiceAdapter`) flushes queued playback and re-starts mic streaming on the existing live socket using the cached `activeConfig`/`emitter`/`captureErrorRef` (nulled in `teardown()`). The reducer's cloud Speaking+bargeIn branch now emits `CancelBrainTurn` + `ResumeCloudUserTurn(epoch, bargeIn)` (local route unchanged: StopLocalSpeaking + StartLocalRecording). The VM's long-lived socket collector reads a `@Volatile sessionEpoch` (updated on open AND resume) instead of a per-open captured epoch, so a transcript arriving on the resumed turn tags the new epoch and passes the guard; `resumeCloudUserTurn` falls back to a fresh `openCloudSession` when `handle.resumeUserTurn()` returns false (socket closed/cancelled). One socket persists across barge-ins.
- **C3 — A tapped notification must re-surface the card even after process death.** `InboundEventCenter.recent` is RAM-only, so after the process is killed a notification tap found nothing (`resolve` returned null → no card). Fix: the notification `PendingIntent` now carries the full event payload as extras (`EXTRA_OPEN_EVENT_KIND/TITLE/BODY/URGENCY/RECEIVED_AT` alongside id/correlation). `MainActivity.handleEventIntent` rebuilds it via `InboundEventCenter.reconstruct(...)` (sourceType=PUSH, null when id/title blank) and hands the whole `InboundEvent` to `PendingInboundEvent` (now carries `InboundEvent?`, not `Pair<id, corr?>`). `FridayEventsViewModel` calls `InboundEventCenter.surfaceFromNotification(event)` which prefers the retained (already-coerced) copy via `resolve`, else re-coerces the reconstructed one through `coercePush`. **Trust boundary:** the extras are attacker-reachable on the exported activity, so re-coercion means a forged CONFIRMATION cold-starts only as a capped (120/400), non-actionable STATUS card — confirm UI stays brain-gate-driven (`awaitingConfirmationState`). `fromPushData`/`reconstruct` share `parseKind`/`parseUrgency` companion helpers. Covered by `InboundEventCenterTest` (reconstruct mapping/drops, surfaceFromNotification prefers-retained + forged-recoerced), `PendingInboundEventTest` (carries InboundEvent), `FridayEventsViewModelTest` (retained resurface + forged cold-starts as STATUS).

Round-2 gates: `:app:compileDebugKotlin` + `:app:testDebugUnitTest` green (**377 tests, 0 failures**). Phase 14 device evidence + FRI-555 verified-inbound-confirmation remain the documented known-open items.

### Things NOT to regress (M2-04)

- Don't resolve the mic route after recording starts. `onMicTap` calls `VoiceRouter.route()` FIRST; `NoVoice`/`NoBrain` open the selector and the mic never records — never capture-then-fail.
- Don't drop the epoch guard in `FridayVoiceTurnMachine`. Every async event carries its launch epoch; a stale epoch must be dropped with zero state/effect output. Removing it re-introduces the "cancelled turn's transcript overwrites the new turn" class of bugs.
- Don't render assist UI or open the mic inside `FridayVoiceInteractionSession`. `onShow` = handoff Intent + `hide()`, nothing else. The assistant services never see audio; all voice runs in the app surface post-lock-chain.
- Don't let `FridaySpeechRecognitionService` touch the singleton `SttRecorder`. It owns its own `AudioRecord` — a live session may hold the singleton. And don't fake recognizer results: real `ERROR_*` codes only.
- Don't make `PendingAssistInvocation` multi-shot or route it through nav args. It's a `@Singleton` one-shot flag consumed exactly once by the Voice screen after the auth/lock chain resolves; `consume()` returns the prior value and clears.
- Don't add an always-on hotword / "Hey Friday" path. Explicitly out of FRI-562 scope.
- Don't let an unverified push render confirmation UI. `coercePush` downgrades PUSH CONFIRMATION → STATUS and caps title/body (120/400); only the brain's armed gate (via `awaitingConfirmationState`) renders `ConfirmationCard`, and the user tap is the only resolver — no timeout auto-confirm, no auto-execute.
- Don't double-surface an event. Foreground → in-app card only; background → notification only. And don't add a full-screen intent to the urgent channel.
- Don't log push payload content. Malformed pushes drop with a content-free log line.
- Don't start `VoiceSessionForegroundService` outside the session-start foreground moment. Android 14+ blocks mic-FGS starts from the background; the service is opt-in (`friday_voice_foreground_continue`) and never sticky.
- Don't double-cancel the cloud brain turn in teardown. `LiveVoiceSession.cancel()` already cancels the in-flight brain turn; `GeminiLiveVoiceAdapter.SessionHandle.cancel()` is a single delegate.
- Don't leave a live cloud session / mic-FGS / lifecycle host alive on a terminal state. Every terminal transition emits `ReleaseSession`; removing it re-leaks the mic indicator and the foreground service after the turn is done/errored. Conversely, don't make LOCAL `BrainDone` emit `ReleaseSession` — that would detach the lifecycle host and cut local TTS mid-utterance. `ReleaseSession` keeps `question`/`answer` visible; only `TeardownAll` (explicit reset) clears them.
- Don't persist an assistant turn under a stale epoch. The `GatewayEvent.Done` persistence is `epochCurrent(epoch)`-guarded and the collect runs in the cancellable `brainTurnJob`; a barge-in/reset that supersedes the turn must not write its late reply.
- Don't double-append the user turn on the cloud route. `LiveCloudBridge.brainTurn` adds `GatewayTurn("user", transcript)` itself; the VM drops the trailing duplicate the context engine already included. Local route passes history verbatim (no drop).
- Don't trust an Intent extra to trigger an assist invocation. `FridayVoiceInteractionSession` sets `PendingAssistInvocation` **in-process** (Hilt EntryPoint, same `:app` process) and `MainActivity` reads no assist extras — this is the forge-proof boundary. And keep the assist consume permission-gated (`onMicNeedsPermission` when mic not granted) so mic-FGS never starts without `RECORD_AUDIO`.
- Don't drop `FridaySpeechRecognitionService`'s caller RECORD_AUDIO check, its Binder-callback try/catch wrapping, or `finish()`/`onCancel` synchronization — they close the confused-deputy and recorder-drain-race holes.
- Don't import provider-specific live types in the VM. The `LiveVoiceAdapter`/`LiveVoiceHandle` seam is the boundary; OpenAI Realtime lands behind the same interface.
- Don't collapse the VM's port seams (`VoiceRoutePort`/`VoiceIo`/`ContextHistorySource`/`VoicePrefsPort`/`VoiceGatewayStatePort`/`VoiceForegroundServicePort`/`SystemHooks`) back into concrete Android types — they're what keep the whole orchestration JVM-testable off-device.
- (B1) Don't let the Voice Gateway (Gemini Live) produce the answer. `brainOwnsAnswer` MUST discard Gemini's own answer audio/`OutputTranscript`, and cloud `TurnComplete` MUST stay non-terminal (it closes the user input turn only). The reply is the Brain Gateway's streamed text, vocalized through `LiveVoiceSynthesizer`/`GeminiTtsSynthesizer` (PCM16 24kHz, direct to the user's Gemini host). `SpeakComplete` is the cloud terminal that emits `ReleaseSession`, NOT `TurnComplete`. Routing the answer back through Gemini Live's own TTS re-introduces the two-brains bug anh Long rejected.
- (B1) Don't send the TTS request anywhere but the user's own Gemini host, and always sanitize its errors with `GatewayErrorSanitizer` before surfacing — the API key rides the URL. No FRIDAY/Firebase proxy on the synthesis path.
- (B2) Don't auto-surface a background-published inbound event. Background publish only retains into `InboundEventCenter.recent` (LRU cap 32) + posts a notification; the card surfaces ONLY when the user taps the notification, routed via `MainActivity.handleEventIntent` → `PendingInboundEvent` → `FridayEventsViewModel` → `resolve()`→`surface()`. `resolve` returns null for an unknown id or a correlation mismatch (both non-null) — don't loosen it to surface forged/unknown ids. Keep `PendingInboundEvent` one-shot.
- (B3) Don't register route/lifecycle listeners before audio focus is granted. `VoiceSessionLifecycleHost.attach` requests focus FIRST and returns false + rolls back on denial; the VM MUST honor false by cancelling the handle/recording, surfacing `FOCUS_DENIED_MESSAGE`, and NOT starting the FGS. Faking a focus grant or starting the mic-FGS on denial is the regression this closes.
- (A1) Don't re-add `VOICE` to `GatewayProvider.OPENAI.roles`. OpenAI Realtime voice is deferred; OpenAI is BRAIN-only. Keep the `LiveVoiceAdapter` seam + contract test so a real OpenAI Realtime adapter can restore it — but don't ship a fake/stub voice path.
- (A2) Don't move the provider status pill back below the orb or wire the controls LEFT button to the drawer. Pill lives in the header center (design parity); controls LEFT is typed-chat (`onToChat`), header LEFT menu is the drawer. Keep `FridayVoiceProviderStatusPill`'s `modifier` param (header needs `weight(1f)`).
- (A3) Don't duplicate the GIF-decode path for the thinking row. `FridayThinkingRow` and `FridayVoiceAnimation` share `AssetGif(assetName, active, modifier)`. Render the row strictly on `uiState is VoiceUiState.Thinking` — it must be absent in every other state.
